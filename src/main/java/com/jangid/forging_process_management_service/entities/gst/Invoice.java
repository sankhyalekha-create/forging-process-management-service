...
public class Invoice {
    ...
    // Work Type - determines which HSN/SAC and terms to use (persisted for invoice numbering logic)
    @Enumerated(EnumType.STRING)
    @Column(name = "work_type", length = 20)
    private WorkType workType;

    // Manual Invoice Flag - indicates invoice not associated with dispatch batches
    @Column(name = "is_manual_invoice")
    @Builder.Default
     ...
     * Unique identifier for e-invoice
     */
    @Size(max = 64)
     ...
     * Contains digitally signed invoice data
     */
